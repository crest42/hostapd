...
merge = merge.rename({'hybrid': 'Hybrid'}, axis=1)
ax = sns.barplot(data=merge.groupby(['algo','run']).agg({'pq_algo': 'first', 'frame_len': 'sum', 'Hybrid': 'first'}).sort_values('frame_len'), y='pq_algo', hue='Hybrid', x='frame_len', ci=0, order = order.reset_index()['algo'])
ax.set(xlabel='Sum of Frame Length in Bytes', ylabel='Algorithm')
plt.tight_layout()
savefig(__file__)
#plt.show()
